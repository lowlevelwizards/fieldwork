...
export class TacticalSteeringService{
  ...
  steer(actor,intent,{game,now=0}={}){
    ...
    const nearbyFriendlies=(game?.actors??[]).filter(other=>other.id!==actor.id&&other.teamId===actor.teamId&&!other.medical?.dead&&distance(actor,other)<96);
    const localCongestion=clamp(nearbyFriendlies.reduce((sum,other)=>sum+clamp((96-distance(actor,other))/96),0)/Math.max(1,(game?.actors??[]).filter(other=>other.id!==actor.id&&other.teamId===actor.teamId&&!other.medical?.dead).length));
    ...
    for(const angleDegrees of angles)for(const factor of factors){
      ...
      let target={x:actor.x+heading.x*step,y:actor.y+heading.y*step};
