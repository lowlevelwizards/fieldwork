...
export class ContactResolutionService{
  ...
  assess({game,observerTeamId,subjectTeamId,relationship="unknown",now=0}={}){
    ...
    const hostile=relationship==="hostile";
    ...
    const prior=this.byPair.get(key);const record={key,observerTeamId,subjectTeamId,relationship,separation,ownCenter,otherCenter,ownRadius,otherRadius,routeConflict,objectiveConflict,mutualAwareness:true,materiallyRelevant,kind,minimumSeparation:Math.max(170,ownRadius+otherRadius+90),assessedAt:now};
