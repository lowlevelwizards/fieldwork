...
function role({id,label,responsibility,selectionReason,preference=()=>0}){
  return Object.freeze({id,label,responsibility,selectionReason,preference});
}

function phase(id,label,reason){
  return Object.freeze({id,label,reason});
}
...
export function getProcedureDefinitionForResponse(responseId){
  return PROCEDURES[responseId]??null;
}

export function getProcedurePhase(definition,phaseId){
  return definition?.phases?.find(item=>item.id===phaseId)??null;
}
