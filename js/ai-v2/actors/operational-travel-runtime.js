...
export class OperationalTravelRuntime{
  constructor({scheduler,brain=null,arbiter=null,decisionLog=null,routeProgress=null}={}){
  ...
  update({game,teamAgenda,teamProcedures,now=0,context={}}={}){
    ...
    for(const actor of game.actors??[]){
      ...
      live.add(actor.id);
      candidates.push({actor,status:refreshed??status,routeIntent});
    }
    ...
    for(const {actor,status,routeIntent} of candidates){
      ...
      const destination=destinationForActor(routeIntent,actors,index);
      ...
      const directive={
        ...
        routeIntent:cloneRouteIntent(routeIntent),destination,acceptanceRadius:70,initialDistance:distance(actor,destination),utilityScore,reason,
        ...
        provenance:{owner:"operational_travel_runtime",source:"campaign_route_intent",teamId:actor.teamId,operationId:actor.operationId,
          ...
          routeMode:status.mode,routeProgress:routeIntent.strategicProgress,routeSegmentIndex:routeIntent.segmentIndex}
      };
      ...
      this.brain?.submit?.({
        actorId:actor.id,action,score:utilityScore,urgency:status.mode==="return"?.58:.44,
        authorityTier:ACTION_AUTHORITY_TIERS.MISSION_RESPONSIBILITY,authorityLabel:"Operation route intent",
        ...
        concernId:staffedConcern?.concernId??null,obligationId:obligation?.id??null,desiredEffect:staffedConcern?.desiredEffect??(status.mode==="return"?"return_toward_origin":"advance_toward_field_mission")
      });
  ...
  get(actorId){
    ...
    return item?{...item,destination:{...item.destination},lookaheadPoint:{...item.lookaheadPoint}}:null;
  }
  summary(){return[...this.byActor.values()].map(item=>({...item,destination:{...item.destination},lookaheadPoint:{...item.lookaheadPoint}}));}
}
