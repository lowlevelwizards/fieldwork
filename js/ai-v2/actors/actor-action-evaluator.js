  ...
  evaluate(context){
    ...
    if(role.fulfillment?.need==="transport_casualty"){
      ...
      if(procedure.phase?.id==="transport_leg"&&evacuation.destination&&procedure.permissions?.drag&&procedure.permissions?.relocate){
        const interactionRange=evacuation.interactionRange??82;
        if((evacuation.distanceToCasualty??Infinity)>interactionRange&&evacuation.approachDestination){
          return[{type:"ApproachEvacuationCasualty",score:1.02,reason:`${role.label} must physically reach the casualty before taking control of transport.`,directive:{...common,reason:`${role.label}: reach the casualty before beginning the secured transport leg`,casualtyId:evacuation.casualtyId,routeId:evacuation.routeId,routeLabel:evacuation.routeLabel,legIndex:evacuation.legIndex,destination:{...evacuation.approachDestination},interactionRange,initialDistance:evacuation.initialApproachDistance,policy:{speedMultiplier:evacuation.approachSpeedMultiplier??.8,arrivalRadius:10,claimSpacing:48}}}];
        }
        return[{type:"EvacuateCasualty",score:1,reason:`${role.label} must satisfy the current transport condition by moving the casualty along the secured route leg.`,directive:{...common,reason:`${role.label}: ${role.responsibility}`,casualtyId:evacuation.casualtyId,routeId:evacuation.routeId,routeLabel:evacuation.routeLabel,legIndex:evacuation.legIndex,waypointId:evacuation.waypoint?.id,waypointLabel:evacuation.waypoint?.label,destination:{...evacuation.destination},interactionRange,initialDistance:evacuation.initialDistance,staminaCost:evacuation.staminaCost,minimumTransportStamina:evacuation.minimumTransportStamina,finalLeg:evacuation.finalLeg,policy:{...evacuation.transportPolicy}}}];
      }
