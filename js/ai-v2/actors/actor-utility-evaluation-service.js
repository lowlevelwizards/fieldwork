...
export class ActorUtilityEvaluationService{
  ...
  evaluate({game,actor,picture,currentAction=null,currentCommitment=null,role=null,agenda=null,now=0}={}){
    ...
    const beliefPressure=clamp(picture?.contactPressure??0);
    const lastProgressAt=moved>9?now:previous.lastProgressAt;
    const lastUsefulAt=purposeful||beliefPressure>=.18||picture?.incomingFire?.length?now:previous.lastUsefulAt;
    ...
    // 3.2F: the tactical picture owns what persistent contact means. Utility
    // consumes that actor-specific belief pressure rather than reconstructing
    // combat relevance from currently visible contacts alone.
    const contactPressure=clamp(beliefPressure+suppression*.24);
    ...
    const candidates=[
      {kind:"react_to_contact",score:clamp(contactPressure*.92+exposure*.18),factors:{beliefPressure,incoming,suppression,exposure}},
      ...
      {kind:"scan",score:clamp(.22+(role?.roleId==="local_security"?.22:.08)+idleFor/30+beliefPressure*.3),factors:{idleFor,beliefPressure}},
    ...
    const result={actorId:actor.id,updatedAt:now,candidates,selected:candidates[0]??null,contactPressure,beliefPressure,treatmentSafety,stagnation,stationaryFor,idleFor,incumbentAction:currentAction?.type??null,commitmentKind:currentCommitment?.kind??null};
    this.byActor.set(actor.id,result);
    actor.aiV2Utility={selected:result.selected?.kind??null,score:result.selected?.score??0,contactPressure,beliefPressure,treatmentSafety,stagnation,candidates:candidates.slice(0,5).map(item=>({kind:item.kind,score:item.score}))};
    return result;
  }
