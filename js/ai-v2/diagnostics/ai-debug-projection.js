...
export function getAIV2DebugSummary(runtime){
    ...
    return `${observers} observing · ${ready} holding ready · ${repositioning} repositioning · ${withdrawing} withdrawing · ${recovery} recovering casualty · ${evacuation} evacuating · ${reporting} reporting · ${issuingWarnings} warning · ${runtime.heardCommunications.count()} heard warning(s) · ${runtime.casualtyKnowledge.count()} casualty record(s) · ${runtime.personalKnowledge.count()} private contact(s) · ${visible} visible · ${activityUpdates} activity update(s) · ${runtime.teamKnowledge.reportCount()} shared report(s) · ${activeEncounterCount} mission-relevant encounter(s) · ${runtime.teamResponses.count()} response(s) · ${runtime.teamConcerns?.count?.()??0} concurrent concern(s) · ${runtime.concernStaffing?.summary?.().length??0} staffed concern slot(s) · ${(runtime.actorObligations?.summary?.()??[]).filter(item=>!["resolved","abandoned"].includes(item.status)).length} actor obligation(s) · ${runtime.teamProcedures.count()} procedure(s) · ${runtime.encounterOutcomes.count()} outcome memory(s) · ${scheduler.activeActions} action(s) · ${runtime.invariants.current.length} invariant issue(s)`;
  }

export function getAIV2DebugDetails(runtime){
    const actionAssignments=runtime.game.actors.map(actor=>{
      const primary=runtime.scheduler.getPrimaryAction(actor.id);
      const role=runtime.teamProcedures.getActorRole(actor.id);
    ...
    const observingActors=runtime.game.actors.filter(actor=>runtime.scheduler.hasAction(actor.id,"ObserveSector"));
    ...
    const communication=runtime.game.actors.map(actor=>{
      const casualtyReport=runtime.scheduler.getAction(actor.id,"ReportCasualty");
    ...
    const sharedContacts=runtime.teamKnowledge.summary().flatMap(entry=>entry.reports.map(report=>{
    ...
    const sharedCasualties=runtime.casualtyKnowledge.summary().flatMap(entry=>entry.casualties.map(report=>{
      const faction=runtime.game.actors.find(actor=>actor.teamId===entry.teamId)?.factionId??entry.teamId;
    ...
    const encounters=runtime.teamEncounters.summary().flatMap(entry=>entry.hypotheses.map(hypothesis=>{
      const faction=runtime.game.actors.find(actor=>actor.teamId===entry.teamId)?.factionId??entry.teamId;
    ...
    const responses=runtime.teamResponses.summary().map(response=>{
      const faction=runtime.game.actors.find(actor=>actor.teamId===response.teamId)?.factionId??response.teamId;
    ...
    const positions=runtime.rolePositions.summary().map(position=>{
      const actor=runtime.game.actors.find(candidate=>candidate.id===position.actorId);
    ...
    const procedures=runtime.teamProcedures.summary().map(procedure=>{
      const faction=runtime.game.actors.find(actor=>actor.teamId===procedure.teamId)?.factionId??procedure.teamId;
    ...
    const activity=runtime.personalKnowledge.summary().flatMap(entry=>entry.contacts.filter(contact=>(contact.track?.activityRevision??0)>0).map(contact=>{
    ...
    const obligations=(runtime.actorObligations?.summary?.()??[]).filter(item=>!["resolved","abandoned"].includes(item.status)).map(item=>`${runtime.game.actors.find(actor=>actor.id===item.actorId)?.name??item.actorId}: ${stateLabel(item.status)} ${stateLabel(item.responsibility)} → ${stateLabel(item.desiredEffect)}`).join(" · ")||"none — no active actor obligations";
    ...
    return{assignment:actionAssignments,personalKnowledge:contacts,activity,communication,teamKnowledge:shared,encounter:encounters,response:responses,concerns,staffing,obligations,procedure:procedures,position:positions,outcome:outcomes};
  }

export function updateAIV2ActorDiagnostics(runtime){
    for(const actor of runtime.game.actors){
      const actions=runtime.scheduler.getActions(actor.id);
      const primary=runtime.scheduler.getPrimaryAction(actor.id);
      ...
      const teamContact=runtime.teamKnowledge.getBestTeamContact(actor.teamId);
      const teamMission=runtime.teamMissions.get(actor.teamId);
      const encounter=runtime.teamEncounters.getBestTeamHypothesis(actor.teamId);
      const response=runtime.teamResponses.get(actor.teamId);
      const teamProcedure=runtime.teamProcedures.get(actor.teamId);
      const procedureRole=runtime.teamProcedures.getActorRole(actor.id);
      const roleAction=runtime.roleActions.get(actor.id);
      const rolePosition=runtime.rolePositions.get(actor.id);
      ...
      const personalCasualty=runtime.casualtyKnowledge.getBestPersonalCasualty(actor.id);
      const receivedCasualty=runtime.casualtyKnowledge.getLatestReceived(actor.id);
      const teamCasualty=runtime.casualtyKnowledge.getBestTeamCasualty(actor.teamId);
      ...
      const actorObligations=runtime.actorObligations?.getActorObligations?.(actor.id)??[];
