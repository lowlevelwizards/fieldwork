...
/**
 * Narrow 3.1H care atom for a staffed casualty obligation.
 * The obligation owns why care is required; this action only owns the physical
 * treatment window once the provider is actually within reach.
 */
export class TreatAssignedCasualtyAction extends AIV2Action{
  constructor({actorId,directive}={}){
    ...
    this.elapsed=0;
    this.duration=Math.max(.8,Number(directive?.duration)||2.6);
  ...
  canStart({game,services}={}){
    ...
    const need=game?.wounds?.getTreatmentNeed?.(casualty)??null;
    if(!need||Number(actor.aiV2MedicalSupplies?.[need.type]??0)<=0)return false;
    const controller=services?.casualtyCare?.getController?.(casualty.id)??null;
    return !controller||controller===actor.id;
  }
  ...
  canContinue({game,services}={}){
    ...
    return Boolean(
      actor&&casualty&&!actor.medical?.dead&&!actor.medical?.unconscious&&!casualty.medical?.dead&&
      obligation&&!["resolved","abandoned"].includes(obligation.status)&&
      distance(actor,casualty)<=Math.max(56,Number(this.directive.interactionRange)||92)&&
      game?.wounds?.getTreatmentNeed?.(casualty)&&this.elapsed<this.duration+.5
    );
  }
  ...
  start(now,{game,services}={}){
    ...
    if(!actor||!casualty)return;
    services?.locomotion?.stop?.(actor,{pose:"medical"});
    actor.operationPausedByEncounter=true;
    actor.currentAction=`Treating ${casualty.name??"casualty"}`;
    actor.workPose="medical";actor.workProp="medical_bag";
    actor.workMedicalItem=game?.wounds?.getTreatmentNeed?.(casualty)?.type??"pressure_dressing";
  ...
  update(delta,{game,services,now=0}={}){
    const actor=game?.actors?.find(candidate=>candidate.id===this.actorId);
    const casualty=game?.actors?.find(candidate=>candidate.id===this.directive.casualtyId);
    if(!actor||!casualty)return{status:"failed",reason:"care_subject_missing"};
    ...
    services?.locomotion?.stop?.(actor,{pose:"medical"});
    services?.attention?.turnToward?.(actor,casualty,delta,{pose:"medical",turnRate:7});
    this.elapsed+=Math.max(0,delta);
    this.progress=Math.min(1,this.elapsed/this.duration);
    actor.currentAction=`Treating ${casualty.name??"casualty"}`;
    actor.aiV2DirectCare={status:"treating",casualtyId:casualty.id,obligationId:this.directive.obligationId,startedAt:this.startedAt,progress:this.progress};
    ...
    this.#release(actor,casualty,services,"completed");
    actor.aiV2DirectCare={status:"completed",casualtyId:casualty.id,obligationId:this.directive.obligationId,completedAt:now,progress:1,treatmentType:result.treatmentType};
    return{status:"completed",reason:"staffed_casualty_treated",data:{casualtyId:casualty.id,treatmentType:result.treatmentType}};
  }

  onInterrupted({game,services}={}){
    const actor=game?.actors?.find(candidate=>candidate.id===this.actorId);
    const casualty=game?.actors?.find(candidate=>candidate.id===this.directive.casualtyId);
    this.#release(actor,casualty,services,"interrupted");
  }
  onCancelled(context={}){this.onInterrupted(context);}

  #release(actor,casualty,services,status){
    if(casualty)services?.casualtyCare?.releasePatient?.(casualty.id,this.actorId);
    if(!actor)return;
    actor.operationPausedByEncounter=false;
    actor.workPose=null;actor.workProp=null;actor.workMedicalItem=null;
    actor.aiV2DirectCare={...(actor.aiV2DirectCare??{}),status};
  }
}
