import { findEntity, getAvailableAction } from "./world-entities.js?v=071-completion-reliability-20260730";
import { transferItem } from "./item-locations.js?v=071-completion-reliability-20260730";
...
export class InteractionSystem {
  ...
  update(delta) {
    if (this.searchingEntityId) {
      ...
      if (!entity || distanceToEntity(this.game.operator, entity) > entity.interactionRadius + 14) {
        this.cancelSearch("Search interrupted");
      } else {
        entity.searchProgress = Math.min(1, entity.searchProgress + delta / entity.searchDuration);
        this.game.operator.searchPose = Math.sin(entity.searchProgress * Math.PI * 7) * .5 + .5;
  ...
  refreshTarget() {
    ...
    const forward = facingVector(operator.facing);
    const candidates = [];
  ...
  trigger() {
    ...
    if (action === "assess") {
      ...
      this.game.emitEvent("assess", entity);
      return true;
    }
    if (action === "use_bandage") {
      const held = this.game.getHeldItem();
      if (held?.condition === "wet") {
        this.game.pushMessage("The dressing is soaked and no longer clean");
        return false;
      }
      return this.game.incident.applyBandage();
    }
    if (action === "give_water") return this.game.incident.giveWater();
    if (action === "assist") return this.game.incident.beginAssist();
    if (action === "release_assist") {
      this.game.assistedActorId = null;
      this.game.pushMessage("Ada sits back down");
      entity.seated = true;
      return true;
    }
    if (action === "install_battery") return this.game.incident.installBattery();
    if (action === "deposit_service_case") return this.game.excursion.depositServiceCase();
    if (action === "review_route") {
      this.game.pushMessage("Follow the marked trail east to the north culvert.", 3);
      return true;
    }
    if (action === "inspect_culvert") return this.game.excursion.inspectCulvert();
    if (action === "clear_debris") return this.game.excursion.clearObstruction();
    if (action === "mark_hazard") return this.game.excursion.markHazard();
    if (action === "talk") {
      this.game.openDialogue(entity);
      return true;
    }
    if (action === "open") {
      entity.state = "opening";
      entity.collision = false;
      this.game.emitEvent("doorOpened", entity);
    ...
    if (action === "close") {
      ...
      this.game.emitEvent("doorClosed", entity);
    ...
    if (action === "close_container") {
      entity.state = "closing";
      this.game.emitEvent("containerClosed", entity);
      return true;
    }
    if (action === "search") {
    ...
    if (action === "take") {
      ...
      if (result.ok) this.game.emitEvent(entity.definitionId === "service_case" ? "heavyItemLifted" : "itemTaken", entity);
      return result.ok;
    }
    return false;
  }
  ...
  cancelSearch(message = null) {
    ...
    this.game.operator.searchTargetId = null;
  ...
  completeSearch(container) {
    ...
    ids.forEach((id, index) => {
      ...
      item.x = container.x + anchor.x;
      item.y = container.y + anchor.y;
      item.groundY = item.y + item.height;
      item.locationType = "world";
    ...
    this.game.pushMessage(ids.length ? `${ids.length} ${ids.length === 1 ? "item" : "items"} found` : "Nothing useful");
    this.game.emitEvent("searchCompleted", container);
  }

  getTarget() {
    return findEntity([...this.game.entities, ...this.game.actors], this.targetId);
  }
}
