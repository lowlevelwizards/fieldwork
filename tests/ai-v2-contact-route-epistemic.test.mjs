...
function makeGame(){
  const operations=new Map([
    ...
    ["opB",{id:"opB",status:"deployed",objectiveId:"objB",routePlan:{waypoints:[{id:"b0",x:0,y:900},{id:"b1",x:700,y:900}],returnWaypoints:[]},actorRouteProgress:{b:{mode:"outbound",index:1,complete:false}}}]
  ]);
  const actors=[
    {id:"a",teamId:"A",operationId:"opA",x:0,y:0,radius:18,medical:{dead:false,unconscious:false}},
    // Hidden ground truth is far away and would not conflict with A's route.
    {id:"b",teamId:"B",operationId:"opB",x:300,y:900,radius:18,medical:{dead:false,unconscious:false}}
  ];
  const livingSandbox={
  ...
  game.aiV2={
    tacticalPictures:{get(actorId){return actorId==="a"?{contactBeliefs:[{subjectTeamId:"B",center:{x:310,y:40},uncertaintyRadius:72,confidence:.76,tacticalSalience:.82,lastConfirmedAt:4,motion:{direction:"east",speed:28}}]}:null;}},
